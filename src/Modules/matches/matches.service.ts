...
import { JWTPayLoadType, UserType } from '../../utils/types';
    ...
    for (const vendor of vendors) {
      try {
        if (!vendor.countries_supported.includes(project.country)) {
          // this.logger.debug(`Vendor ${vendor.id} skipped: does not support country ${project.country}`);
          continue;
        }
...
private async sendNotification(project: any, newMatchesCount: number, totalMatchesCount: number): Promise<void> {
   ...
   */
  // public async rebuildMatches(projectId: number, payload: JWTPayLoadType): Promise<{ length: number; matches: Match[]; totalMatchesCount: number; message?: string }> {
  public async rebuildMatches(projectId: number, payload: JWTPayLoadType): Promise<{ length: number; matches: { id: number; projectId: number; vendorId: number; vendorName: string; score: number; createdAt: Date; notifiedAt: Date; is_sla_expired: boolean }[]; totalMatchesCount: number; message?: string }> {
    ...
    try {
      ...
      return sortedMatches.length > 0
        ? { length: sortedMatches.length, matches: sortedMatches, totalMatchesCount }
        : { length: 0, matches: [], totalMatchesCount, message: 'No matches found for this project' };
    } catch (error) {
   ...
   */
  async getAllMatches(query: any): Promise<{ status: string; results: number; pagination: any; data: Match[] }> {
    return this.factory.getAll(query, Resource_Name.MATCHES, Models_Name.MATCH);
  }
   ...
   */
  async getProjectMatches(projectId: number, payload: JWTPayLoadType): Promise<Match[]> {
    try {
      const project = payload.userType === UserType.ADMIN
        ? await this.projectsService.getProjectByIdWithRelationsInternal(projectId)
        : await this.projectsService.getProjectByIdWithRelations(projectId, payload);
   ...
   */
  public async deleteMatch(id: number, payload: JWTPayLoadType): Promise<{ message: string }> {
